...
import { logUpiTransaction } from '@/lib/firebase';
...
function DonationDialog({ amount, onClose }: { amount: string; onClose: () => void }) {
	...
	// Log UPI transaction when payment is initiated
	const handlePayment = async () => {
		try {
			// Log as 'initiated'
			await logUpiTransaction({
				upiId: 'getepay.usfbqrap255469',
			...
			console.error('Failed to log UPI transaction:', error);
		}
	};

	// After payment, you can add a button for the user to confirm success/failure:
	const handleStatusUpdate = async (status: 'success' | 'failed') => {
		await logUpiTransaction({
			upiId: 'getepay.usfbqrap255469',
			amount: Number(amount),
			donorName,
			...
			<DialogContent>
				...
				<div className="text-center my-4">
					...
					{/* <div className="flex gap-2 justify-center mt-4">
						<Button variant="success" onClick={() => handleStatusUpdate('success')}>
							Mark as Successful
						</Button>
					...
					{donationTiers.map((tier) => (
						<Card key={tier.amount} className="text-center flex flex-col bg-card/80 backdrop-blur-sm">
							<CardHeader className="items-center">
								<div className="p-4 bg-accent rounded-full mb-4">{tier.icon}</div>
								<CardTitle className="font-headline text-4xl">₹{tier.amount.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</CardTitle>
								<CardDescription className="text-base">{tier.description}</CardDescription>
							</CardHeader>
							<CardFooter className="mt-auto">
								<Button className="w-full" onClick={() => handleDonationClick(tier.amount)}>
									Donate ₹{tier.amount.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
								</Button>
							</CardFooter>
						</Card>
					))}
				...
				<Card className="mt-12 max-w-lg mx-auto bg-card/80 backdrop-blur-sm">
					<CardHeader>
						...
						<CardDescription className="text-center text-base">
							<span className="font-headline text-xl">Bank Details:</span>
							<br />
							Name: Shri Gopal Krishna Gaushala Sewa Trust
							<br />
